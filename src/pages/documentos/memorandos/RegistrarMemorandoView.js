...
const RegistrarMemorandoView = () => {
    ...
    const postdata = async (e) => {
        ...
        const [res, ok, message] = await fetchPostData("/documento/memorando", data)
        if (!ok) {
            alert(message)
            return
        }
    ...
    return (
        <div className="container mb-4">
            <div>
                <p className="h5 ms-3 mt-4 mb-4">Generar Memorando</p>
                <form className="form me-5" onSubmit={postdata}>
                    <FormInput title="Codigo"
                        name='codigo'
                        value={men.codigo}
                        onChange={change}
                    />
...
export default RegistrarMemorandoView
